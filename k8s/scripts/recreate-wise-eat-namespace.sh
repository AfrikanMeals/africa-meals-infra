...
echo "Si ECONNREFUSED : republier Docker sur cni0 (--env-file OBLIGATOIRE) :"
echo "  cd /opt/wise-eat/mongodb && sudo K3S_CNI_GATEWAY=10.42.0.1 docker compose --env-file .env.mongodb up -d"
echo "  cd /opt/wise-eat/redis && sudo K3S_CNI_GATEWAY=10.42.0.1 docker compose --env-file .env.redis up -d"
echo "  cd /opt/wise-eat/memcached && sudo K3S_CNI_GATEWAY=10.42.0.1 docker compose --env-file .env.memcached up -d"
echo "  sudo /opt/wise-eat/scripts/ufw-allow-k3s-pods.sh"
echo "  Vérifier : ss -tlnp | grep -E '10\\.42\\.0\\.1:(27017|6379|6380|11211)'"
