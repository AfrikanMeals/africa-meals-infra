#!/usr/bin/env bash
# nginx k8s.wise-eat.com → Headlamp NodePort 30850 (HTTPS, token Headlamp uniquement).
# Usage : sudo ./install-k8s-nginx.sh
set -euo pipefail
...
export K8S_DASHBOARD_DOMAIN K8S_DASHBOARD_BACKEND_HOST K8S_DASHBOARD_BACKEND_PORT CERTBOT_WEBROOT

if [[ -f "/etc/letsencrypt/live/${K8S_DASHBOARD_DOMAIN}/fullchain.pem" ]]; then
  ensure_letsencrypt_nginx_tls_files
  envsubst '${K8S_DASHBOARD_DOMAIN} ${K8S_DASHBOARD_BACKEND_HOST} ${K8S_DASHBOARD_BACKEND_PORT} ${CERTBOT_WEBROOT}' \
  ...
  log "nginx HTTP k8s → :${K8S_DASHBOARD_BACKEND_PORT}"
fi
...
log "Headlamp : https://${K8S_DASHBOARD_DOMAIN}/ (auth = token SA uniquement, pas basic auth nginx)"
