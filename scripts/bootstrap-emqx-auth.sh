#!/usr/bin/env bash
# Bootstrap utilisateurs MQTT EMQX (built_in_database) via API REST (Bearer token).
set -euo pipefail
...
ensure_auth_chain() {
  ...
  resp="$(emqx_api_code -X POST "${EMQX_API}/authentication" \
...
upsert_mqtt_user() {
  local user="$1" pass="$2"
  local create_payload update_payload resp code body
  ...
  if emqx_api "${EMQX_API}/authentication/${AUTH_ID_ENC}/users/${user}" >/dev/null 2>&1; then
    resp="$(emqx_api_code -X PUT "${EMQX_API}/authentication/${AUTH_ID_ENC}/users/${user}" \
      -H 'Content-Type: application/json' \
      -w '\n%{http_code}' \
      -d "${update_payload}")"
    code="${resp##*$'\n'}"
    [[ "${code}" == "200" ]] || die "PUT user ${user} → HTTP ${code} : ${resp%$'\n'*}"
    log "EMQX user mis à jour : ${user}"
    return 0
  fi

  resp="$(emqx_api_code -X POST "${EMQX_API}/authentication/${AUTH_ID_ENC}/users" \
    -H 'Content-Type: application/json' \
    -w '\n%{http_code}' \
    -d "${create_payload}")"
  code="${resp##*$'\n'}"
  [[ "${code}" == "200" || "${code}" == "201" ]] || die "POST user ${user} → HTTP ${code} : ${resp%$'\n'*}"
  log "EMQX user créé : ${user}"
}

log "Attente API EMQX + login (:${EMQX_DASHBOARD_PORT})…"
wait_for_emqx_api_login || die "API EMQX injoignable ou login échoué sur 127.0.0.1:${EMQX_DASHBOARD_PORT}"
